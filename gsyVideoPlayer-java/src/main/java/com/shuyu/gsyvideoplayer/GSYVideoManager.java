...
public class GSYVideoManager implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnCompletionListener,
    ...
    public void onBufferingUpdate(final IMediaPlayer mp, final int percent) {
        mainThreadHandler.post(new Runnable() {
            ...
            public void run() {
                if (listener != null) {
                    long mDurationUS = mp.getMediaInfo().mMeta.mDurationUS;
//                    if (mDurationUS<1000 * 20 * 1000){
                    if (false){
                        listener().onBufferingUpdate(buffterPoint);
                    }else{
                        if (percent >buffterPoint){
                            Log.e(TAG, "false="+mDurationUS+"run: percent="+percent+"||buffterPoint="+buffterPoint);
                            listener().onBufferingUpdate(percent);
                        } else {
                            Log.e(TAG,"true="+ mDurationUS+"run: percent="+percent+"||buffterPoint="+buffterPoint);
                            listener().onBufferingUpdate(buffterPoint);
                        }
                    }
                }
