...
//            ArrayList<String> paths = groupImage.mGroupMap.get(dirName);
            ArrayList<MediaModel> paths = groupImage.mGroupMapMedia.get(dirName);
    ...
    private class GroupImageViewHolder extends RecyclerView.ViewHolder{
      ...
      /*  void bindItem(String dirName, ArrayList<String> paths){
        ...
        void bindItem(String dirName, ArrayList<MediaModel> models){
            dirNameText.setText(dirName);
            photoSizeText.setText(String.format(mContext.getString(R.string.pick_photo_size),models.size() + ""));
            Glide.with(mContext).load(Uri.parse("file://" + models.get(0).getThumPath())).into(cover);
            itemView.setTag(R.id.pick_dir_name,dirName);
        }
