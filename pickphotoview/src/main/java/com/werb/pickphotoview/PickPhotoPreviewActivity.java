...
public class PickPhotoPreviewActivity extends AppCompatActivity {
    ...
    private ArrayList<MediaModel> allMedia;
    private ArrayList<MediaModel> selectMedia;
    private MediaModel path;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        path = (MediaModel) getIntent().getSerializableExtra(PickConfig.INTENT_IMG_PATH);
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(PickPhotoPreviewActivity.this);
//        String stringExtra = getIntent().getStringExtra(PickConfig.INTENT_IMG_LIST);
        String stringExtra = preferences.getString(PickConfig.MEIDA_PREVIEW_MODEL,"");
        allMedia = new Gson().fromJson(stringExtra,new TypeToken<List<MediaModel>>(){}.getType());
        imageViews = new ArrayList<>();
        selectMedia = PickHolder.getMeidas();
        for (int i = 0; i < 4; i++) {
    ...
    private void initView() {
        ...
        for (int i = 0; i < allMedia.size(); i++) {
            if (allMedia.get(i).getPath().equals(path.getPath())){
                indexOf = i;
                break;
            }
        }
        judgeSelect(allMedia.get(indexOf));
        ...
        viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                MediaModel mediaModel = allMedia.get(position);
                judgeSelect(mediaModel);
            }
    ...
    private class listPageAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(final ViewGroup container, final int position) {
            ...
            String path = allMedia.get(position).getPath();
            if(path.endsWith(".gif")) {
                ...
                container.addView(gif,params);
                Glide.with(PickPhotoPreviewActivity.this).load(allMedia.get(position).getThumPath()).into(gif);
                /*PlayerView player;
                String url = allMedia.get(position).getFile().getAbsolutePath();
                View view = View.inflate(container.getContext(),R.layout.simple_player_view_player,null);
                player = new PlayerView(PickPhotoPreviewActivity.this,view)
                        .setTitle("什么")
                        ...
                        .showThumbnail(new OnShowThumbnailListener() {
                            ...
                            public void onShowThumbnail(ImageView ivThumbnail) {
                                Glide.with((Activity) container.getContext())
//                                        .load("http://pic2.nipic.com/20090413/406638_125424003_2.jpg")
                                        .load(allMedia.get(position).getThumPath())
                                        .into(ivThumbnail);
                            }
                        })
                        .setPlaySource(url)
    ...
    private void judgeSelect(final MediaModel path){
        int indexOf = selectMedia.indexOf(path);
        if(indexOf != -1){
        ...
        myToolbar.setRightLayoutOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(misSelect){
                    myToolbar.setRightIcon(R.mipmap.pick_ic_un_select_black);
                    selectMedia.remove(path);
                    PickHolder.setSelectedMedia(selectMedia);
                    misSelect = false;
                }else {
                    if(selectMedia.size() < pickData.getPickPhotoSize()) {
                        myToolbar.setRightIconDefault(R.mipmap.pick_ic_select);
                        selectMedia.add(path);
                        PickHolder.setSelectedMedia(selectMedia);
                        misSelect = true;
                    }else {
    ...
    private void finishForResult(){
        ...
        intent.putExtra(PickConfig.INTENT_IMG_LIST_SELECT, selectMedia);
